export default function Navbar() {
  return (
    <header className="navbar sticky top-0 z-1 bg-base-200/80 backdrop-blur supports-[backdrop-filter]:bg-base-200/60">
      {/* Left section – hamburger (only visible on mobile) */}
      <div className="flex-none lg:hidden">
        <label htmlFor="drawer" className="btn btn-square btn-ghost">
          {/* DaisyUI hamburger */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-6 w-6"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor">
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={1.5}
              d="M4 6h16M4 12h16M4 18h16"
            />
          </svg>
        </label>
      </div>

      {/* Search bar */}
      <div className="flex-1 px-2 lg:px-4">
        <div className="form-control w-full">
          <input
            type="text"
            placeholder="Buscar…"
            className="input input-bordered w-full sm:w-80"
          />
        </div>
      </div>

      {/* Desktop menu */}
      <nav className="hidden flex-none lg:flex">
        <div className="dropdown dropdown-hover dropdown-bottom dropdown-center">
          <div tabIndex={0} role="button" className="btn btn-ghost normal-case">
            Explorer
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth="1.5"
              stroke="currentColor"
              className="size-4">
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="m19.5 8.25-7.5 7.5-7.5-7.5"
              />
            </svg>
          </div>
          <ul
            tabIndex={0}
            className="dropdown-content menu rounded-box z-10 w-52 bg-base-100 p-2 shadow-sm">
            <li>
              <a>Item 1</a>
            </li>
            <li>
              <a>Item 2</a>
            </li>
          </ul>
        </div>
      </nav>

      {/* Language + avatar */}
      <div className="flex flex-row gap-2">
        <div className="dropdown dropdown-end hidden sm:block">
          <label tabIndex={0} className="btn btn-ghost normal-case">
            ES
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth="1.5"
              stroke="currentColor"
              className="size-4">
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="m19.5 8.25-7.5 7.5-7.5-7.5"
              />
            </svg>
          </label>
          <ul
            tabIndex={0}
            className="dropdown-content menu rounded-box w-24 bg-base-100 p-2 shadow">
            <li>
              <a>ES</a>
            </li>
            <li>
              <a>EN</a>
            </li>
          </ul>
        </div>
        <div className="avatar placeholder border rounded-full">
          <div className="w-10 rounded-full bg-neutral-focus text-neutral-content">
            <img src="/5611.jpg" alt="avatar" />
          </div>
        </div>
      </div>
    </header>
  );
}
